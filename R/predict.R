...
#' @export
augment_marginal_lt2 <- function(object, newdata = NULL, re.form.conditional = NULL, re.form.marginal = NULL,  f = compose(c_to_f, unstd_ftemp), p = 0.5, lt_names = "LT50", center = median, se_fit = NULL, std_ftemp_seq = seq(-2, 2, by = 0.005), parallel = T, B = 100) {
  # beta = NULL,
  # if(is.null(beta))
  if(parallel)
    `%fdo%` <- `%dopar%`
  ...
  lt_samples <- foreach(i = 1:nrow(newdata), .export = c("object","newdata", "re.form.conditional", "re.form.marginal", "p", "std_ftemp_seq"), .packages = c("coldhardiness")) %fdo% {
    ...
    if(is.null(se_fit) || !se_fit) {
      ...
      sapply(p, function(p_i) std_ftemp_seq[which(p_marginal > p_i)[1]])
    } else {
      p_conditional <- array(NA, dim = c(dim(lp), B))
      for(j in 1:ncol(lp)) {
        re <- matrix(rnorm(nrow(lp)*B, 0, rowSums(object$sflat[,dimnames(object$sflat)[[2]] %in% re.form.marginal])), ncol = B)
...
#' @export
augment_marginal_lt3 <- function(object, newdata = NULL, re.form.conditional = NULL, re.form.marginal = NULL, vc.form.marginal = NULL,  f = compose(c_to_f, unstd_ftemp), p = 0.5, lt_names = "LT50", center = median, se_fit = NULL, std_ftemp_seq = seq(-2, 2, by = 0.005), parallel = T, B = 100) {
  # beta = NULL,
  # if(is.null(beta))
  if(parallel)
    `%fdo%` <- `%dopar%`
